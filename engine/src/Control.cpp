...
#include "roomcut_mach_cleanup.h"
...
kern_return_t requestReply(mach_port_t servicePort,
    ...
    if (kr != KERN_SUCCESS) {
        roomcut_destroy_unsent_message(req, kr);
    ...
    mach_port_mod_refs(self, replyPort, MACH_PORT_RIGHT_RECEIVE, -1);
    if (kr == KERN_SUCCESS && !normalizeControlReply(*buf, msgId)) {
...
kern_return_t controlGetComparison(mach_port_t servicePort, uint32_t timeoutMs,
    ...
    if (result == KERN_SUCCESS) *outReply = buffer.comparisonReply;
    return result;
}
...
kern_return_t controlReplyComparison(const RoomcutGetComparisonRequest& request,
                                     RoomcutComparisonReply reply) {
    if (request.header.msgh_remote_port == MACH_PORT_NULL) return KERN_INVALID_ARGUMENT;
    reply.header.msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_MOVE_SEND_ONCE, 0);
    reply.header.msgh_size = sizeof(reply);
    reply.header.msgh_remote_port = request.header.msgh_remote_port;
    reply.header.msgh_local_port = MACH_PORT_NULL;
    reply.header.msgh_voucher_port = MACH_PORT_NULL;
    reply.header.msgh_id = ROOMCUT_MSG_GET_COMPARISON;
    reply.msgType = ROOMCUT_MSG_GET_COMPARISON;
    reply.version = ROOMCUT_COMPARISON_VERSION;
