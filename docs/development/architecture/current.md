...
2026-09-09 작업 트리 기준입니다. 파일별 전체 원문과 최신 행 수는 [생성 목록](../generated/README.md)에 있습니다.
행 수는 조사 대상을 찾는 지표이며 행 수만으로 God object 여부를 판정하지 않습니다.
...
드라이버의 실시간 IO는 공유 링에 씁니다. 연결 작업은 별도 작업 스레드가 담당합니다.
앱과 엔진의 명령 경로는 기존 Mach와 C shim을 그대로 둡니다. 별도 IPC를 추가하지 않습니다.
...
| `RoomcutViewModel.swift` (960행) | 편집 연결·UI 게시·타이머와 IO. 폴링 한 번의 판단은 `RefreshPlanner` | 남은 것은 뷰별 표시 상태뿐 | 늦은 읽기·쓰기가 새 값/이름/오류를 덮어쓰지 않음; UI 토큰·동작 유지 |
...
| `NowPlayingMonitor.swift` (873행) | 보조 프로세스 수명·소스 결정. 프레이밍/시간은 `NowPlayingStream`, 가사 수명은 `NowPlayingLyrics` | 보조 프로세스 수명. 주입 가능한 헬퍼 실행기가 먼저 필요 | 표시 우선순위·취소·재연결 일치 |
| `NowPlayingView.swift` (1443행) | 재생 정보와 가사의 여러 표시 구성 | 재사용되는 표시 요소의 경계부터 검토 | 큰 뷰 파일이라는 이유만으로 무조건 분할하지 않음 |

책임은 한 번에 하나씩 분리합니다. 이전 경로의 특성 검사를 확보하고 호출자를 옮긴 뒤 동일 검사를 실행합니다.
단순 확장 파일로 옮겨 같은 전역 상태를 계속 공유한다면 책임 분리의 완료로 세지 않습니다.
파일 수 증가와 간접 호출 비용도 검토합니다. 한 번만 쓰는 범용 서비스 계층을 만들지 않습니다.
