...
확인일: 2026-09-08. 아래 링크를 웹에서 직접 열었습니다. 외부 자료 원문 전체를 저장하지 않으며,
근거와 Roomcut의 구현 선택을 구분합니다. 이후 관련 작업을 시작할 때 링크와 내용의 변화를 다시 확인합니다.
...
| [GNU Mach 메시지 전송](https://www.gnu.org/software/hurd/gnumach-doc/Message-Send.html) | 권한을 운반하는 메시지와 전송 오류 처리 | 포트 소유권 감사의 보조 자료 | GNU 설명만으로 Darwin 동작을 단정하지 않고 실제 macOS Mach 검사 수행 |
...
이전 성능·A/B 개발 때 인용한 ITU K 가중 필터, Apple 파라미터 램프, DSP Concepts 평활화 자료는
...
[Apple 액터 설명](https://developer.apple.com/videos/play/wwdc2021/10133/)의 await 사이 상태 재검증 원칙을
직접 선택/임시 복구의 소유권에 적용했습니다. TSan과 별도로 지연된 실제 호출 순서·UI 요청값을 검사합니다.
...
동기 세션 작업은 직렬 큐로 보내고 취소 뒤에도 이미 시작한 작업의 정리 완료를 기다립니다.
...
같은 샘플레이트 형식 변환과 SRC 호출 방식 차이를 확인했습니다. `RoomTunePCM`은 입력 샘플레이트를 유지하며 첫 채널을 보존합니다.
[REW 측정 지침](https://www.roomeqwizard.com/betahelp/help/html/makingmeasurements.html)과
[클록 조정 안내](https://www.roomeqwizard.com/betahelp/help/html/analysis.html)도 확인했습니다.
...
명시적인 인터페이스와 암묵적인 전역 상태 전달 지양 원칙을 확인했습니다.
`CoreAudioDevices`에는 장치 ID·값·조회 결과를 전달하며 엔진 연결이나 상태를 숨겨서 조회하지 않습니다.
공개 C 변환과 기존 Mach 연결은 호출자에 남겼습니다. 지침 준수를 파일 행 수 감소만으로 판정하지 않습니다.
...
IO-02에서도 같은 계약을 기본 출력·샘플레이트·밸런스 쓰기에 적용했습니다.
...
새 자료만 모으지 말고 검증 가능한 구현/실험으로 연결합니다. 자료가 바뀌어도 기존 UI를 임의 재설계하지 않습니다.
