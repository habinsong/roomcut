...
public:
    ...
    // Update the envelope without reallocating or clearing the look-ahead.
    void setReleaseMs(double releaseMs) {
        releaseCoeff_ = std::exp(-1.0 / std::max(1.0, releaseMs * 0.001 * fs_));
    }
    ...
    inline void processFrame(float* frame) {
        ...
        // log10 is needed only when a meter is read, not on every audio frame.
    ...
    double maxGainReductionDb() const {
        return minGain_ < 1.0 ? -20.0 * std::log10(minGain_) : 0.0;
    }
