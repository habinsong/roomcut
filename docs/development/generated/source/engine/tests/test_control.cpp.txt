...
void* serverThread(void* arg) {
    ...
    roomcut::EngineSoundState sound(roomcut::ChainParams::flat(), "flat");
    for (int i = 0; i < 9; ++i) {
        ...
        CHECK(roomcut::normalizeControlRequest(buf), "real Mach request satisfies the control envelope");
        switch (buf.raw.header.msgh_id) {
            ...
            case ROOMCUT_MSG_SET_COMPARISON: {
                const auto& request = buf.comparisonRequest;
                const auto reference = roomcut::decodeParameters(request.reference);
                uint32_t status = 0;
                if (request.kind == ROOMCUT_COMPARISON_PRESET) {
                    if (!sound.applyPreset(request.presetId, &reference, request.enabled != 0)) status = 1;
                } else sound.applyComparison(roomcut::decodeParameters(request.current), reference, request.enabled != 0);
                CHECK(roomcut::controlReplyAck(request.header, ROOMCUT_MSG_SET_COMPARISON, status) == KERN_SUCCESS,
                      "comparison acknowledgement fits the transport");
                break;
            }
            case ROOMCUT_MSG_GET_COMPARISON: {
                RoomcutComparisonReply reply{};
                reply.enabled = sound.settings().enabled;
                reply.revision = sound.settings().revision;
                std::snprintf(reply.presetId, sizeof(reply.presetId), "%s", sound.presetID());
                roomcut::encodeParameters(sound.settings().current, reply.current);
                roomcut::encodeParameters(sound.settings().reference, reply.reference);
                CHECK(roomcut::controlReplyComparison(buf.getComparison, reply) == KERN_SUCCESS,
                      "large comparison reply includes both complete presets");
                break;
            }
            default:
...
int main() {
    roomcut::EngineSoundState identity(roomcut::ChainParams::flat(), "flat");
    auto other = roomcut::ChainParams::flat(); other.preampDb = -3;
    identity.applyComparison(identity.parameters(), other, true);
    identity.applyComparison(identity.parameters(), other, false);
    CHECK(std::strcmp(identity.presetID(), "flat") == 0, "utility toggles preserve the current preset identity");
    identity.apply(identity.parameters(), identity.presetID());
    CHECK(std::strcmp(identity.presetID(), "flat") == 0, "reusing the current label does not overlap its destination");
    ...
    std::snprintf(comparison.presetId, sizeof(comparison.presetId), "%s", "night");
    auto reference = roomcut::ChainParams::flat();
    reference.preampDb = -7;
    ...
    CHECK(roomcut::controlGetComparison(service, 2000, &comparisonReply) == KERN_SUCCESS,
          "client reads a comparison reply larger than the former receive buffer");
    CHECK(comparisonReply.enabled == 1 && comparisonReply.revision == 1 && std::strcmp(comparisonReply.presetId, "night") == 0,
          "comparison control state is authoritative");
    CHECK(roomcut::decodeParameters(comparisonReply.reference) == reference, "all reference fields survive the Mach round trip");
    ...
    CHECK(roomcut::controlGetComparison(service, 2000, &comparisonReply) == KERN_SUCCESS
          ...
          && roomcut::decodeParameters(comparisonReply.reference) == reference,
          "a failed preset operation cannot partially replace the comparison reference");
