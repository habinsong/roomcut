...
사운드 컨트롤을 끝까지 펼치면 A/B 비교와 실행 취소 행이 나타납니다.
A와 B는 EQ·공간 효과·다이내믹스·매크로 위치를 각각 보관합니다. 복사 버튼은
현재 설정을 반대쪽 상태에 복사합니다. 비교 상태와 이력은 현재 실행 동안 유지됩니다.

⌘Z로 실행을 취소하고 ⇧⌘Z로 다시 실행합니다. 한 번의 드래그는 한 번에
되돌아가며, A와 B는 각각 최대 100단계의 이력을 보관합니다. 이름이나 숫자를
입력 중일 때는 텍스트 실행 취소가 우선합니다.
