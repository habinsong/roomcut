...
- **リミッターとゲイン。** プリアンプ・出力トリムとピークリミッターで、強い EQ カーブでも出力がクリップしないようにします。音量の平準化ノブは大きい音と小さい音の差を抑え、夜間のリスニングに向いています。
...
- **プリセット。** Signature・Apple 製品・Speakers・Headphones に分類された内蔵ライブラリと、自分で保存したプリセット。JSON ファイルとして書き出し・読み込みができ、出力デバイスごとにプリセットを記憶して自動で切り替えることもできます。
...
- **Settings.** 出力デバイスとフォーマット、音量、デバイス別プリセット、ログイン時に起動、外観（テーマ・レイアウト・言語）、プリセットの書き出し/読み込み、歌詞キャッシュ。
...
- **Advanced** は 5 つのサブタブで全機能を開きます:
  ...
  - **Limiter.** ピークリミッターとプリアンプ・出力ゲイン、夜間リスニング向けの音量の平準化。
...
このリポジトリにサードパーティのソースは同梱していません。Roomcut が参照した技法やサービスのクレジット、およびプリセットやスクリーンショットに登場する機器名の商標表示は [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md) にあります。両ファイルはすべてのリリースに同梱されるため、ダウンロードしたビルドにも必ず含まれます。

Roomcut は Now Playing のメタデータを Apple の非公開 MediaRemote フレームワーク経由で読み取ります。そのため Mac App Store には配信できず、macOS のアップデートで Now Playing パネルが動作しなくなる可能性があります（オーディオ経路は動作し続けます）。
