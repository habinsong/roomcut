...
static void test_width_is_frequency_dependent() {
    ...
    // Frequency-dependent M/S: treble side takes the full width (×1.9), bass side
    // only 30% (stays near-centred) → widen affects highs much more than lows.
    ...
    CHECK_NEAR(trebleRatio, 1.9, 0.06, "width +100 widens treble side ~x1.9");
    CHECK(bassRatio < 1.45, "width +100 keeps bass side near-centred");
    CHECK(trebleRatio > bassRatio + 0.3, "width widens treble more than bass");
...
static void test_allpass_decorrelation_widens_mono() {
    // A mono source has zero side, so a plain side-gain can't widen it. The
    // all-pass decorrelation must synthesise some side on widen — yet stay
    // mono-safe: summing L+R back must reproduce the (gain-matched) mono input.
    Spatial spatial;
    spatial.prepare(48000.0);
    spatial.setParams(100.0, 0.0, 0.0, 0.0);   // full widen, speaker mode
    double sideSq = 0.0, sumErrSq = 0.0;
    const int n = 4096;
    for (int i = 0; i < n; ++i) {
        ...
        const double side = (frame[0] - frame[1]) * 0.5;
        sideSq += side * side;
        const double monoOut = (frame[0] + frame[1]) * 0.5;  // midGain == 1 here
        sumErrSq += (monoOut - m) * (monoOut - m);
    }
    CHECK(std::sqrt(sideSq / n) > 1.0e-3, "decorrelation creates side from a mono source");
    CHECK(std::sqrt(sumErrSq / n) < 1.0e-4, "decorrelation is mono-safe (L+R preserves mono)");
}
...
int main() {
    ...
    test_allpass_decorrelation_widens_mono();
