...
struct RoomTuneTab: View {
    ...
    @ObservedObject var measurement: RoomTuneMeasurement
    ...
    var body: some View {
        ...
        .onDisappear { measurement.cancel() }
    }
    ...
    private var measurementStatusText: String {
        ...
        case .preparing:               return L("준비 중", "Preparing", "準備中", "Préparation", "Vorbereitung")
        case .stopping:                return L("정리 중", "Finishing", "終了処理中", "Finalisation", "Abschluss")
    ...
    private func startMeasurement() {
        guard let deviceID = scanner.deviceID else { return }
        ...
        measurement.start(deviceID: deviceID, beforeStart: {
            AppLaunch.traceRoomTune("override begin")
            let accepted = await bypass.begin()
            AppLaunch.traceRoomTune("override begin result=\(accepted)")
            return accepted
        }) {
            AppLaunch.traceRoomTune("override restore")
            let restored = await bypass.restore()
            AppLaunch.traceRoomTune("override restore result=\(restored)")
            return restored
        }
    }
